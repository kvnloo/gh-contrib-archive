# Autodevelop — gh-contrib-archive
...
| | |
|---|---|
| **This repo** | https://github.com/kvnloo/gh-contrib-archive |
| Clone | `git clone https://github.com/kvnloo/gh-contrib-archive.git` |
...
1. Clone or open https://github.com/kvnloo/gh-contrib-archive. `cd` into that tree. `git fetch origin`. `python3 .verified-oss-loop/rollout.py show`. Branch from `origin/$(python3 .verified-oss-loop/rollout.py get worker_base)` unless the issue names another base. Day-pass PRs target `feature_target`. Overnight PRs target `overnight_target`.
...
```bash
gh issue list --repo kvnloo/gh-contrib-archive --label claimable --state open
gh pr list --repo kvnloo/gh-contrib-archive --state open
```
...
8. Open a PR on https://github.com/kvnloo/gh-contrib-archive at `feature_target` (day) or `overnight_target` (overnight) with `.github/PULL_REQUEST_TEMPLATE.md` filled. Bind evidence to `head_revision`. **Do not merge `main` or `dev`.**
