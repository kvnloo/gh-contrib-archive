...
const smoke = process.env.EXPERIENCE_SMOKE === '1';
await fs.mkdir(out, { recursive: true });
const browser = await chromium.launch({ headless: false, args: ['--disable-dev-shm-usage', '--ignore-gpu-blocklist', '--enable-webgl', '--disable-background-timer-throttling', '--disable-renderer-backgrounding'] });
const results = [];
let cases = baseline ? [{ id: 'baseline', width: 960, height: 540, look: 'midnight' }] : [
...
if (smoke && !baseline) cases = cases.filter(s => s.id === 'midnight' || s.mobile);
try {
  for (const spec of cases) {
    ...
    if (!baseline) await page.waitForFunction(() => document.querySelector('canvas')?.dataset.sceneReady === 'true', undefined, { timeout: 90000 });
    await page.waitForTimeout(baseline ? 3000 : 1000);
    // Stop resubmitting identical capture frames. Interaction tests below use a fresh,
    // unmodified page and verify the actual application's pause/resume behavior.
    ...
    await page.screenshot({ path: path.join(out, `${spec.id}-ui.png`), timeout: 90000 });
  ...
  if (!baseline) {
    ...
    await page.waitForFunction(old => document.querySelector('canvas')?.dataset.sceneTime !== old, a);
    await page.getByRole('button', { name: 'Pause dream', exact: true }).tap();
    await page.waitForTimeout(500); const b = await clock(); await page.waitForTimeout(600); assert.equal(await clock(), b, 'Pause freezes actual scene');
    ...
    await page.evaluate(() => { window.requestAnimationFrame = () => 0; });
    await page.screenshot({ path: path.join(out, 'phone-controls.png'), timeout: 90000 });
    await page.goto(`${base}/`, { waitUntil: 'networkidle', timeout: 90000 });
    await page.getByRole('button', { name: /Filters/ }).tap();
    await page.getByPlaceholder('search public text').fill('latency');
    await page.getByRole('button', { name: 'Filter', exact: true }).tap();
    assert.equal(new URL(page.url()).searchParams.get('q'), 'latency', 'Phone filtering updates URL');
